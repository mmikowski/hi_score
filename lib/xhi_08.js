  ...
  function mainFn () {
    ...
    postObj.flowObj.exec(
      function _01GetChangeList () {
        ...
        // Handler for close
        function on_close_fn ( exit_code ) {
          if ( exit_code === 0 ) {
        ...
        child_obj.stdout.on( 'data', on_stdin_fn.bind( 'stdout' ) );
        child_obj.stderr.on( 'data', on_stdin_fn.bind( 'stderr' ) );
        child_obj.on( 'close', on_close_fn );
      },
      ...
      function _checkSpacingFn () {
        ...
        for ( idx = 0; idx < jsCount; idx++ ) {
          filename = jsList[ idx ];
          bound_fn = on_grep_fn.bind( { filename : filename } );
          promise_obj = postObj.grepFileFn( filename, /\t|\s+$/ );
          promise_obj.then( bound_fn ).catch( catchFn );
      ...
      function _makeTodoListFn () {
        ...
        xhiObj.childProcObj.exec(
          ...
          function ( error_obj, stdout ) {
            if ( error_obj ) {
              xhiObj.warnFn( '    Error getting TODO list' );
              return catchFn();
            }
      ...
      function _checkTodoFn () {
        var
          ...
          line_list = [
            ...
            '===== Please review the list of outstanding TODOs =====',
        ...
        xhiObj.askFn( 'Is this acceptable? (Y/n) ', function ( str ) {
          if ( str.match( /^[n]/i ) ) {
            return catchFn( 'Exiting at user request' );
          }
          next_fn();
        });
      },
