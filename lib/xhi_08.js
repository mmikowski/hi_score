 ...
 *
 * Use eslint settings in package.json/eslintConfigure
*/
/*global*/
'use strict';
...
function lintFn () {
  ...
  function mainFn () {
    ...
    postObj.flowObj.exec(
      function _01GetChangeList () {
        ...
        function onstdin_fn ( data ) {
          ...
          LINE: for ( i = 0; i < line_count; i++ ) {
            ...
            clean_str = line_str.replace( /^\s*(modified|new):\s*/, '' );
            ...
            if ( clean_str.match( /\.js$/ )
              && ! clean_str.match( /^js\/vendor/ )
              && ! clean_str.match( /^dist/ )
            ) {
              jsList.push( clean_str );
            }
          }
      ...
      function _ESLintFilesFn () {
        var
          then_fn     = this,
          local_str   = '    02: Check files with ESLint: ',
          child_obj
          ;
        ...
        child_obj = xhiObj.makeSpawnObj(
          xhiObj.fqModuleDirname + '/.bin/eslint', jsList
        );
      ...
      function _makeTodoListFn () {
        ...
        xhiObj.childProcObj.exec(
          ...
          function ( error_obj, stdout ) {
            ...
            todoList = stdout[ 'replace' ]( /(:\d+:).*TODO /g, '$1\n  ' )
              .split( '\n' );
