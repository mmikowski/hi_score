...
function buildFn () {
  ...
  function copyPathsFn () {
    var
      ctx_obj    = this,
      row_count  = deployPathList.length,
    ...
    PATH: for ( idx = 0; idx < row_count; idx++ ) {
      ...
      if ( ! tgt_pathname ) {
        tgt_pathname = xhiObj.pathObj.basename( src_pathname );
      }
  ...
  function mainFn () {
    ...
    postObj.flowObj.exec(
      ...
      function _11_04_copyPathsFn () {
        logFn( '  04: Copy subtrees...' );
        copyPathsFn.call({ then_fn: this, catch_fn : failFn });
      },
      function _11_05_cleanupFn () {
        logFn( '  05: Cleanup...' );
