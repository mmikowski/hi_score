...
function buildFn () {
  ...
  function _02_makeUglyMapsFn () {
    ...
    if ( js_concat_str ) {
      ...
      uglyJsMap.concat = js_concat_str;

      // Save concat for debugging
      if ( ! uglyJsMap.code ) {
        xhiObj.makeWritePromFn(
          fqMetaDirname + '/' + app_id + '-ugly-map.json',
          JSON.stringify( uglyJsMap )
        );
  ...
  function _03_saveUglyFilesFn () {
    ...
    if ( uglyJsMap ) {
      write_list.push(
        [ fqMetaDirname + '/' + app_id + '-concat.js', uglyJsMap.concat ]
      );
      write_list.push(
